...
#define USBD_PID_FS_DFU     57105
#define USBD_PRODUCT_STRING_FS_DFU     "STM32 DownLoad Firmware Update"
#define USBD_CONFIGURATION_STRING_FS_DFU     "DFU Config"
#define USBD_INTERFACE_STRING_FS_DFU     "DFU Interface"
...
uint8_t* USBD_FS_ProductStrDescriptor(USBD_SpeedTypeDef speed, uint16_t* length)
{
  ...
  else if (main_usb.class == USB_CLASS_DFU) {
    if (speed == 0)
    {
      USBD_GetString((uint8_t*)USBD_PRODUCT_STRING_FS_DFU, USBD_StrDesc, length);
    }
    else
    {
      USBD_GetString((uint8_t*)USBD_PRODUCT_STRING_FS_DFU, USBD_StrDesc, length);
...
uint8_t* USBD_FS_ConfigStrDescriptor(USBD_SpeedTypeDef speed, uint16_t* length)
{
  if (main_usb.class == USB_CLASS_CDC) {
    ...
    if (speed == USBD_SPEED_HIGH)
    {
      USBD_GetString((uint8_t*)USBD_CONFIGURATION_STRING_FS_DFU, USBD_StrDesc, length);
    }
    else
    {
      USBD_GetString((uint8_t*)USBD_CONFIGURATION_STRING_FS_DFU, USBD_StrDesc, length);
...
uint8_t* USBD_FS_InterfaceStrDescriptor(USBD_SpeedTypeDef speed, uint16_t* length)
{
  if (main_usb.class == USB_CLASS_CDC) {
    ...
    if (speed == 0)
    {
      USBD_GetString((uint8_t*)USBD_INTERFACE_STRING_FS_DFU, USBD_StrDesc, length);
    }
    else
    {
      USBD_GetString((uint8_t*)USBD_INTERFACE_STRING_FS_DFU, USBD_StrDesc, length);
...
//	added preinitialization for descriptors (change values in global table):
void USBD_Descriptor_preinit(usb_class_e class)
{
  ...
  case USB_CLASS_MSC:
    ...
    break;
  case USB_CLASS_DFU:
    main_usb.class = USB_CLASS_DFU;

    USBD_FS_DeviceDesc[4] = 0x00;                             /*bDeviceClass DFU*/
    USBD_FS_DeviceDesc[5] = 0x00;                             /*bDeviceSubClass DFU*/
    USBD_FS_DeviceDesc[10] = LOBYTE(USBD_PID_FS_DFU);         /*idProduct*/
    USBD_FS_DeviceDesc[11] = HIBYTE(USBD_PID_FS_DFU);         /*idProduct*/
    break;
  default:
    break;
  }
