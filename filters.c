...
//	Allocate memory for arrays
	iir->buffer_input = (float *)malloc(sizeof(iir->buffer_input)*(iir->filter_order+1));
	iir->buffer_output = (float *)malloc(sizeof(iir->buffer_output)*(iir->filter_order));
	iir->forward_coefficients = (float *)malloc(sizeof(iir->forward_coefficients)*(iir->filter_order+1));
	iir->feedback_coefficients = (float *)malloc(sizeof(iir->feedback_coefficients)*(iir->filter_order));
...
//DO POPRAWY DYTUACJA GDY BUFFER index is max
