...
static void SPI_transmit_one(uint8_t data);
static void SPI_receive(uint8_t *data, uint16_t size);
static void SPI_receive_one(uint8_t *data);
...
static void OSD_update_logo_characters();
...
static void SPI_transmit(uint8_t *data, uint16_t size)
{
	...
	while (i < size)
	{
		...
		SPI2->DR = data[i]; // second and following data sending as soon as TX flag is set
		i++;
		time_flag6_1 = get_Global_Time();
		while (!((SPI2->SR) & SPI_SR_TXE))
		{
...
static void SPI_receive(uint8_t *data, uint16_t size)
{
	while (size > 0)
	{
		...
		SPI2->DR = OSD_DUMMY_BYTE; // send anything - IMPORTANT!
		time_flag6_1 = get_Global_Time();
...
void OSD_SPI_write(uint8_t instruction, uint8_t data)
{
	...
	SPI_transmit_one(instruction);
	SPI_transmit_one(data);
...
void OSD_SPI_read(uint8_t address, uint8_t *memory_address)
{
	...
	// if you want read more bytes use SPI_receive() function
	CS_OSD_enable();
	SPI_transmit_one(address | 0x80); // for reading adress|0x80
	SPI_receive_one(memory_address);
	CS_OSD_disable();
}
...
{ //	You can NOT print 255th character with this function!
	...
	CS_OSD_enable();
	SPI_transmit(character_number_tab, string_length);

	//	terminate auto-increment mode:
	SPI_transmit_one(OSD_TERMINATE_AUTO_INCREMENT);
	CS_OSD_disable();
}
